...
# Animated heatmap
animated_heatmap <- ggplot(heatmap_data, aes(x = variable, y = Attack_type, fill = value)) +
...
# Render animation
animate(animated_heatmap, nframes = 100, fps = 10, width = 800, height = 600, renderer = gifski_renderer())
```
...
# Animated heatmap with log values and added legend
animated_heatmap <- ggplot(heatmap_data, aes(x = flow_bin, y = Attack_type, fill = value)) +
  geom_tile(color = "white") +
  geom_text(aes(label = round(value, 2)), color = "black", size = 3) +  # Add dynamic text labels
  scale_fill_gradient(
    low = "lightblue",
    high = "orange",
    name = "Log Bandwidth\n(bytes/sec)",  # Legend title
    labels = function(x) paste0("~10^", round(x, 1))  # Show log scale representation
  ) +
  labs(
    title = "Heatmap of Log-Transformed Metrics by Attack Type",
    subtitle = "Flow Duration Bin: {closest_state}",
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_blank(),  # Remove x-axis numeric values
...
# Render animation
animate(animated_heatmap, nframes = 100, fps = 5, width = 800, height = 600, renderer = gifski_renderer())
...
# Data preparation using flow_duration as the x-axis
data <- data %>%
...
animated_flow_lines <- ggplot(data, aes(x = flow_duration, y = log_bandwidth, color = Attack_type, group = Attack_type)) +
  ...
  scale_color_viridis_d() +  # Use a distinct color palette
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "bottom"
  ) +
  coord_cartesian(xlim = c(0, 50)) +  # Manually adjust x-axis range
  transition_reveal(flow_duration)  # Gradually reveal the lines along flow_duration

# Render animation with increased frames for smoother drawing
animate(animated_flow_lines, nframes = 500, fps = 10, width = 1200, height = 800, renderer = gifski_renderer())
